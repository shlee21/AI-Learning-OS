...
Learning OS는 결과 뒤에 회고를 덧붙이는 기능이 아닙니다. 질문의 적합성을 점검하고, 모르는 개념을 판단에 필요한 지식으로 연결하며, 사람이 선택·거부·검수한 경험을 재사용 가능한 명시지로 바꾸는 학습 사이클입니다.

```text
질문 → 이해 → 인간 판단 → 결과 → 검수 → 명시지화 → 더 나은 질문
```
...
현재 버전은 `v0.4.0`입니다. 문서 기반 규칙 세트이며 특정 모델이나 서비스에 종속되지 않습니다.
